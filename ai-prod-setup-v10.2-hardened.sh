#!/usr/bin/env bash
# =============================================================================
# ENTERPRISE PRODUCTION GRADE - AI STACK DEPLOYMENT v10.2 (HARDENED)
# Target: Ubuntu 22.04 / 24.04 LTS (Server)
# 
# v10.2 HARDENING:
# - PINNED software versions for stability (no more 'latest' tags)
# - Privacy-focused DNS (removed Google fallback, added Cloudflare/OpenDNS)
# - Local network firewall restriction for internal services
# - Added comprehensive health checks with fallback verification
# - Auto-detect local subnet for homelab use
# v10.1 FIXES RETAINED:
...
# CONFIGURATION
# -----------------------------------------------------------------------------
SCRIPT_VERSION="10.2.0-hardened"
...
FallbackDNS=1.1.1.1#cloudflare-dns.com 208.67.222.222#resolver1.opendns.com
DNSSEC=yes
Cache=yes
EOF
...
# LOCAL_NETWORK variable for homelab restriction
# Detect primary network interface and local subnet
LOCAL_INTERFACE=$(ip route | grep default | awk '{print $5}' | head -1)
LOCAL_SUBNET=$(ip -4 addr show "$LOCAL_INTERFACE" 2>/dev/null | grep -oP 'inet \K[0-9.]+/[0-9]+' | head -1)
if [[ -z "$LOCAL_SUBNET" ]]; then
    log_warn "Could not detect local subnet, using 192.168.1.0/24 as default"
    LOCAL_SUBNET="192.168.1.0/24"
fi
log_info "Local network detected: $LOCAL_SUBNET"

# SSH allowed from anywhere (needed for remote management)
ufw allow 22/tcp comment 'SSH'

# HTTP/HTTPS allowed from anywhere (for reverse proxy/external access if needed)
ufw allow 80/tcp comment 'HTTP'
ufw allow 443/tcp comment 'HTTPS'

# Internal services restricted to local network only (PRIVACY FOCUSED)
ufw allow from "$LOCAL_SUBNET" to any port 9090 proto tcp comment 'Prometheus - Local Only'
ufw allow from "$LOCAL_SUBNET" to any port 3000 proto tcp comment 'OpenWebUI - Local Only'
ufw allow from "$LOCAL_SUBNET" to any port 3001 proto tcp comment 'Grafana - Local Only'
ufw allow from "$LOCAL_SUBNET" to any port 4242 proto tcp comment 'Agent-Zero - Local Only'
...
SERVICES=("ollama" "openwebui" "prometheus" "grafana" "agent0")
FAILED_SERVICES=()

for svc in "${SERVICES[@]}"; do
    check_container_health "$svc" || FAILED_SERVICES+=("$svc")
done

# Report status
if [[ ${#FAILED_SERVICES[@]} -gt 0 ]]; then
    log_error "Failed services: ${FAILED_SERVICES[*]}"
    log_warn "Check logs: docker compose logs <service-name>"
    log_warn "To restart: docker compose restart <service-name>"
else
    log_success "All services started successfully!"
fi

# Additional Ollama health check (API endpoint)
sleep 5
if curl -s http://localhost:11434/api/tags > /dev/null 2>&1; then
    log_success "Ollama API responding"
else
    log_warn "Ollama API not responding yet - may need more time for large models"
fi
